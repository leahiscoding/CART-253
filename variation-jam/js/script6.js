...
function keyPressed() {
  ...
  if (
    key === "3" &&
    (gameState === "start" || gameState === "play" || gameState === "end")
  ) {
...
function checkScore() {
  ...
  if (score === maxScore) {
    // if total amount of the flies caught is 15, player wins
    finishState = "win"; // player wins
    gameState = "end"; // change game state to end
    return;
  }
  if (kermitHit >= 10) {
    // if kermit gets hit 10 times, player loses
  ...
  if (timer.timePassed > timer.timeInterval) {
    // if timer exceeds time limit, player loses
    finishState = "lose"; // player loses
    gameState = "end"; // change game state to end
    return;
  }
